...
def extract_brand(brand_model_str: str, url_str: str = "") -> str:
    """
    Extracts the brand slug from brand_model or URL.
    """
    text = ""
    if brand_model_str:
        text += " " + str(brand_model_str).lower()
    if url_str:
        text += " " + str(url_str).lower()
        
    text = text.replace("-", "_").replace(" ", "_")
    
    if "dena" in text or "دنا" in text:
        return "dena"
    elif "santafe" in text or "santa_fe" in text or "سانتافه" in text:
        return "hyundai_santafe-ix45"
    elif "jac_s5" in text or "jac" in text or "جک" in text or "s5" in text:
        return "jac_s5"
    elif "cerato" in text or "سراتو" in text:
        return "kia_cerato"
    elif "mvm_x22" in text or "x22" in text:
        return "mvm_x22"
    elif "206" in text or "۲۰۶" in text:
        return "peugeot_206"
    elif "pars" in text or "پارس" in text:
        return "peugeot_pars"
    elif "pride" in text or "پراید" in text:
        return "pride_131"
    elif "quick" in text or "کوییک" in text:
        return "quick"
    elif "tondar" in text or "تندر" in text or "l90" in text or "ال_90" in text or "ال90" in text:
        return "renault_tondar-90"
...
def clean_car_record(raw_data: dict) -> dict:
    ...
    # 7. Add engineered features required by MLP Neural Network
    year_val = cleaned.get("year")
    if year_val is not None:
        car_age = 2026 - year_val
        if car_age < 0:
            car_age = 0
    else:
        car_age = 0
    cleaned["car_age"] = float(car_age)
    
    mileage_val = cleaned.get("mileage", 0)
    cleaned["yearly_mileage"] = float(mileage_val) / (car_age + 1)
    
    insurance_val = cleaned.get("insurance", 0)
    cleaned["has_insurance"] = 1 if insurance_val > 0 else 0
    
    cleaned["brand"] = extract_brand(raw_data.get("brand_model", ""), raw_data.get("url", ""))
